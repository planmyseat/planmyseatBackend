...
import generateExcelSeatingPlan from "../utils/generateExcelSeatingPlan.js";
...
export const markAttendance = async (req, res) => {
  const { presentStudents, className } = req.body;
...
export const requestExcel = async (req, res) => {
  const { id } = req.params;
  const user = req.user;

  try {
    const plan = await SeatingPlan.findOne({ _id: id, createdBy: user._id }).select("-__v -createdAt -createdBy")
    if (!plan) return res.status(404).json({ message: "plan not found" })
    const block = await Block.findOne({_id: plan.blockId})

    const excelBuffer = await generateExcelSeatingPlan(plan, block.block);

    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    
    res.setHeader('Content-Disposition', `attachment; filename="SeatingPlan"`);
    res.setHeader('Content-Length', excelBuffer.length);

    return res.send(excelBuffer);
  } catch (error){
    console.error('Error in requestExcel:', error);
    return res.status(500).json({ message: 'Failed to generate Excel file due to a server error.', error: error.message });
  }
}